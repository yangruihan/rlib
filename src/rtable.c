...
static Entry* findEntry(Table* t, int cap, uint32_t key)
{
    ...
    for(;;)
    {
        Entry* entry = TABLE_DATA_OFFSET(t, index);
...
static void adjustCap(Table* t, int cap)
{
    ...
    for (size_t i = 0; i < t->capacity; i++)
    {
        ...
        Entry* dest = findEntry(t, cap, entry->key);
        dest->key = entry->key;
...
bool table_get(Table* t, uint32_t key, void* val)
{
    ...
    Entry* entry = findEntry(t, t->capacity, key);
